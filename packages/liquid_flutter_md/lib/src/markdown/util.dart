...
extension StringExtensions on String {
  ...
  DedentedText dedent([int length = 4]) {
    ...
    return DedentedText(substring(start), tabRemaining, start);
  }
  ...
  /// Converts this string to a list of [Line], with each [Line] carrying the
  /// cumulative [Line.charOffset] of its first character in the original string.
  List<Line> toLines() {
    final lines = <Line>[];
    var offset = 0;
    for (final content in LineSplitter.split(this)) {
      lines.add(Line(content, charOffset: offset));
...
class DedentedText {
  ...
  /// The number of leading characters removed from the original line during
  /// dedenting. Callers can add this to a [Line.charOffset] to get the
  /// adjusted source position of the dedented content.
  final int stripCount;

  DedentedText(this.text, this.tabRemaining, [this.stripCount = 0]);
}
