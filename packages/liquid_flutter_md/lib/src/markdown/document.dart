...
class Document {
  ...
  ///
  /// [baseOffset] is added to all source offsets produced by [InlineParser],
  /// allowing callers to map inline positions back to the original document.
  List<Node> parseInline(String text, {int baseOffset = 0}) =>
      InlineParser(text, this, baseOffset: baseOffset).parse();
    ...
    for (var i = 0; i < nodes.length; i++) {
      ...
      if (node is UnparsedContent) {
        final inlineNodes = parseInline(
          node.textContent,
          baseOffset: node.sourceOffset ?? 0,
        );
        nodes.removeAt(i);
...
extension _ElementExt on Element {
  static Element footnoteAnchor(String ref, int i) {
    ...
    return Element('a', [
      ...
      // <data-footnotes aria-label="Back to content">.
