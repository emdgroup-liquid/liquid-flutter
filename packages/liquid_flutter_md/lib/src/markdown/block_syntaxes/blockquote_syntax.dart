...
class BlockquoteSyntax extends BlockSyntax {
  ...
  List<Line> parseChildLines(BlockParser parser) {
    ...
    while (!parser.isDone) {
      ...
      if (match != null) {
        ...
        childLines.add(Line(
          currentLine.content.substring(markerEnd),
          charOffset: currentLine.charOffset != null
              ? currentLine.charOffset! + markerEnd
              : null,
        ));
        parser.advance();
        _lazyContinuation = false;
      ...
      if ((otherMatched is ParagraphSyntax &&
        ...
        // Lazy continuation lines already carry their original charOffset.
  ...
  Node parse(BlockParser parser) {
    // Capture the offset of the first line before consuming child lines.
    final elementOffset = parser.current.charOffset;
    ...
    final element = Element('blockquote', children);
    if (elementOffset != null) {
      element.sourceOffset = elementOffset;
      // sourceLength: sum of all child lines' content + ">" markers + newlines.
      // Cheapest approximation: use the charOffset of the next unconsumed line
      // minus elementOffset.  We don't have that here, so leave length null for
      // now — the controller falls back to raw source when length is null.
    }
    return element;
  }
}
