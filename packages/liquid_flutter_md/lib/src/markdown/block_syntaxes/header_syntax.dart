...
class HeaderSyntax extends BlockSyntax {
  ...
  Node parse(BlockParser parser) {
    ...
    final lineOffset = parser.current.charOffset ?? 0;
    final lineLength = parser.current.content.length;

    parser.advance();

    final element = Element('h$level', [
      if (content != null)
        UnparsedContent(content)
          ..sourceOffset = lineOffset + openMarkerEnd
          ..sourceLength = content.length,
    ]);
    element.sourceOffset = lineOffset;
    element.sourceLength = lineLength;
    return element;
  }
}
