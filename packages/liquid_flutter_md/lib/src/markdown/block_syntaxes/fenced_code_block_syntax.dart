...
class FencedCodeBlockSyntax extends BlockSyntax {
  ...
  Node parse(BlockParser parser) {
    ...
    // Capture source offset of the opening fence line before parseChildLines
    // advances past it.
    ...
    pre.sourceOffset = elementOffset;
  ...
  List<Line> parseChildLines(
    ...
    while (!parser.isDone) {
      ...
      if (closingFence == null ||
        ...
        final strippedContent = _removeLeadingSpaces(
          parser.current.content,
          upTo: indent,
        );
        final spacesRemoved =
            parser.current.content.length - strippedContent.length;
        childLines.add(
          Line(
            strippedContent,
            charOffset: parser.current.charOffset != null
                ? parser.current.charOffset! + spacesRemoved
                : null,
          ),
        );
        parser.advance();
