...
class InlineParser {
  ...
  /// Added to all source offsets, mapping positions back to the original doc.
  ...
  InlineParser(this.source, this.document, {this.baseOffset = 0}) {
  ...
  void _processDelimiterRun(int bottomIndex) {
    ...
    while (currentIndex < _delimiterStack.length) {
      ...
      if (openerIndex > bottomIndex && openerIndex > openerBottom) {
        ...
        // Capture offsets before close() consumes opener/closer nodes.
        final openerSourceOffset = openerTextNode.sourceOffset;
        final closerSourceOffset = closerTextNode.sourceOffset;
        final closerSourceLength = closerTextNode.sourceLength;
        ...
        // Annotate the produced element(s) with source offsets.
        if (openerSourceOffset != null &&
            closerSourceOffset != null &&
            closerSourceLength != null) {
          final totalLength =
              closerSourceOffset + closerSourceLength - openerSourceOffset;
          for (final n in nodes!) {
            if (n is Element && n.sourceOffset == null) {
              n.sourceOffset = openerSourceOffset;
              n.sourceLength = totalLength;
            }
          }
        }
  ...
  void _combineAdjacentText(List<Node> nodes) {
    for (var i = 0; i < nodes.length - 1; i++) {
      ...
      if (node is Text && nodes[i + 1] is Text) {
        ...
        final firstOffset = firstNode.sourceOffset;
        ...
        final combined = Text(buffer.toString());
        if (firstOffset != null) {
          combined.sourceOffset = firstOffset;
          // Sum the sourceLength of each node in the run.
          var totalLength = 0;
          for (var k = i; k < j; k++) {
            final n = nodes[k] as Text;
            totalLength += n.sourceLength ?? n.textContent.length;
          }
          combined.sourceLength = totalLength;
        }
        nodes[i] = combined;
  ...
  void writeText() {
    ...
    final node = Text(text)
      ..sourceOffset = baseOffset + start
      ..sourceLength = pos - start;
    _tree.add(node);
    start = pos;
  }
