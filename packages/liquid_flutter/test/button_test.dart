...
void main() {
  ...
  testWidgets("LdButton with icon", (WidgetTester test) async {
    var key = const Key("trailing");
    var key2 = const Key("leading");
    ...
    await test.pumpWidget(LdThemeProvider(
        ...
        child: Directionality(
          textDirection: TextDirection.ltr,
          child: LdButton(
            leading: Icon(LucideIcons.plus, key: key2),
            onPressed: () {},
            trailing: Icon(LucideIcons.plus, key: key),
            child: const Text("Test button"),
          ),
        )));
    await test.pumpAndSettle();
    expect(find.byKey(key), isNotNull);
    expect(find.byKey(key2), isNotNull);
  });
