...
void main(List<String> args) {
  final parser = ArgParser()
    ..addOption(
      'proto-dir',
      defaultsTo: 'lib/proto',
      help: 'Directory containing .pb.dart files',
    )
    ..addOption(
      'config',
      defaultsTo: 'lib/proto/identifiable_proto.json',
  ...
  if (parsed['help'] as bool) {
    print('''
Post-processes .pb.dart files to add Identifiable mixin to message classes.

Usage: dart run liquid_flutter:identifiable_proto [options]
...
''');
  ...
  if (verbose && config.fieldOverrides.isNotEmpty) {
    print('Loaded ${config.fieldOverrides.length} field override(s) from $configPath');
  }

  final files = dir.listSync().whereType<File>().where((f) {
    final name = p.basename(f.path);
    return name.endsWith('.pb.dart') &&
        !name.contains('.pbgrpc.') &&
        !name.contains('.pbjson.') &&
        !name.contains('.pbenum.');
  }).toList();

  if (files.isEmpty) {
    print('No .pb.dart files found in $protoDir');
    exit(0);
  }
  ...
  for (final file in files) {
    ...
    final transformed = transformIdentifiableProto(content, config.fieldOverrides);
    ...
    if (transformed != content) {
      ...
      final name = p.basename(file.path);
      final classCount = _countMessageClasses(content);
      if (classCount == 0) {
        print('$name: skipped (no GeneratedMessage classes)');
      } else {
...
int _countMessageClasses(String content) {
  final re = RegExp(r'extends\s+\$pb\.GeneratedMessage');
