...
- **Primary navigation**: Share `LdNavigationTab` destinations between `LdTabNavigation` (mobile/bottom) and `LdNavigationRail` (desktop drawer). App chooses chrome; set `drawerWidth` / `drawerMinWidth` from `LdNavigationRail.defaultWidth` / `defaultMinWidth` for rails. Hierarchical drawers use `LdDrawerItemSection` instead.
